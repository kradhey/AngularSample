...
export class ProfileService {
    ...
    getBehindTheScene()
    {
        const url=environment.endpoints.profile.freelancer.behindTheScene
        return this.http.get<ISiteApiResponse>(url).map(r=>this.onGetSuccessful(r))
  ...
  private onGetOrganizations(response: ISiteApiResponse) {
    
        return response.data as Organizations[]  || null;
      }
    
      getOrganizationDetail(orgPassCode: string,org:OrganizationsEnum) {
        const url = environment.endpoints.profile.freelancer.organizationDetail(orgPassCode,org);
        return this.http
          .get<any>(url, {})
